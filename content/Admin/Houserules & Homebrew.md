...
- **Modified Aid**. Also known as Friedport Aid. The DC is 20 again instead of 15, but the tiers of success are more favourable. Critical success gives +3, success gives +2, failure gives a +1 instead of nothing, but a critical failure still gives a -1. Being a Master in the skill increases each tiers bonus by 1, including nullifying critical failure into doing nothing.
- **Godpowder vs Redpowder.** If a character has firearms, with the exception of air guns, they ought to be a Titan worshipper of some sort. This will grant them access to crafting and using godpowder (ammunition) and firearms as usual. Others who pick up a Titan-Worshippers firearm cannot create new godpowder for it (and usually cannot buy it, as they lack a good source). They can still use it for the amount remaining, but the Firearm is capable of [Misfiring](https://2e.aonprd.com/Rules.aspx?ID=1649), which you have to roll a DC 5 flat check for after you miss with it. You may also incur a deity's [[GM-Side Deity Content|wrath]] for using a stolen gun. A character can have access to a worse version of firearms even as a non-religious person, using the alchemical by-product called Redpowder. These firearms can also misfire like above, but only after a Critical Miss. No amount of maintenance will remove these flat checks.
...
- **Student of Lore.** Using a skill training to get a lore gives it to you as the "Additional Lore" skill feat. See above.
- **Premaster Clarifications**. The remaster Pathfinder 2e has set some new standards, but not all content has been re-printed or errata'd to use them yet. Here's some clarifications:
...
- **Weapons Drawn?** More of a clarification on assumptions than a houserule. We do not assume you start with your weapons drawn in every encounter. Your character can be expected to wield their weapon ahead of time when in a dangerous environment or situation, but not just about whenever. If you are caught by surprise or in a place where wielding a weapon isn't kosher, you'll have to spend actions to draw your weapon. This goes both ways though - catch enemies by surprise and they'll have to draw theirs!
- **Guns Loaded?** You can keep a gun pre-loaded with ammunition even when holstered (don't try this at home). We usually allow this with crossbows too. You can always keep your weapons with ammunition loaded if you're in a weapons drawn type situation, as above. 
...
- **Hero Point sources**. Hero Points can be gained from a variety of sources like fulfilling in-game objectives, but we usually add some more consistent "meta" sources as well. In the Landcrab Plot, this was spelled out as:
...
- **Support Moves** - call in special support actions in boss fights from allies you've earned.
...
- **Guide Books.** Finding a tome on a certain topic can provide an item bonus to downtime activities relevant to the book, for activities in which consulting a book throughout is possible and convenient. The Item Bonus can easily be at least 1 over what general items of the party's level can get due to its specificity, like an early game book offering a +2 bonus to crafting a certain item or set of items, or an invaluable ancient tome giving a +4.
- **Formula Schematics.** Formulas can be found as loot called Schematics, just as they can be bought. A character can learn it by spending downtime with it if it is an item they could craft, but the item doesn't just disappear when it does (even if that would be convenient game design). The schematic can be sold as a treasure item for full value (GM's need to consider it as part of the loot value they give out). Creating a copy of a formula is possible, but uses the rules for Crafting and the Earn Income table, as no formula is a simple schematic.
- **Crafting Lores.** Potentially more of a clarification than a homebrew. So long as you meet the prerequisites in Crafting to Craft an item otherwise, you can use a Lore like "Glassmaking" to craft said item, gaining a reduction in the DC for it. Lores like "Blacksmithing," "Alchemy," "Magical Crafting," "Clockwork" are so broad and broadly useful it can only ever give a -2 DC. Specific Crafting Lores like the rest found in [Specialty Crafting](https://2e.aonprd.com/Feats.aspx?ID=5216) and similar are specific and niche enough they can give the full -5 DC. If only a component of the item being crafted makes use of the specific lore, the GM may give you half the DC reduction (5->2, 2->1).
...
- **Mook Nexuses.** Spawns mooks over time.
...
- **Riskier Firearms.** Firearms may be made more of a gamble to use, to increase their potential threat level and relevance as a major plot-point of the setting. They gain the Risky trait.
  [Risky] Gain an additional damage die as a baseline (this doesn't increase the amount you gain from striking runes, which still only add 1 extra per tier). If it has the Kickback trait, it deals 2 additional damage instead of 1. If it has the Deadly or Fatal trait, each adds 1 more damage die than usual on a critical hit. If it has Scatter, the damage dealt is 1 point plus 1 for every damage die. As a trade-off, the weapon can always [misfire ](https://2e.aonprd.com/Rules.aspx?ID=1649), which you have to roll a DC 5 flat check for on a Critical Failure. If you have a Redpowder Weapon or a stolen Titan-Worshipper's firearm as a non-worshipper, you have to roll on a failure as well.
> A GM can also introduce a Risky firearm as just a special version of an item, instead of as a whole rule. This might be a way to play-test the rule.
- **Rogues use Backstabber**. The existence of the [Backstabber](https://2e.aonprd.com/Traits.aspx?ID=544) trait has always been cool, but disappointing since Rogues and Investigators usually bypass it through their already existing precision damages. This rule would make Backstabber increase precision damage by 1 (or 2 when a +3 weapon) if applicable.
- **Slow Healing**. Splits healing into 3 types - circumstance (medicine skill treatment), status (magic) and item (alchemical and consumable). When you get healed for one type, you gain it as a condition whose number of healing you note down. If you were to get healed for the same type again, you subtract the amount of healing from the source you already had, essentially only allowing you to "increase" the amount gained and not "add" to it. Whatever new source of healing you overrides your condition. This means you can only be healed so much from each type before you need downtime to start a fresh. 
  The conditions are removed either during a Rest or during a Safe Rest, which can only be done in a town or otherwise safe base (but not while travelling). 
	**Consistent Healing.** Intended for Slow Healing but can be used without it. All healing sources have their dice taken out of them and are turned into a flat number instead, usually their average. Allows for more consistency and simplifying the process of healing over a number of days, and reduces re-rolling to try to get better results (and being disappointed when it's wasted).
	**Consumable Healing Doubled.** For Slow Healing and Consistent Healing. To make consumable healing items more worthwhile when they can't be used again and again. Consumable healing items with die involved give maximum healing.
...
![[Pasted image 20260830145836.png]]![[Pasted image 20260830145849.png]]![[Pasted image 20260830145858.png]]
![[Pasted image 20260830145907.png]]
...
- **Twin Duo Archetype.** Allows playing as two characters using many mechanics from the Simulacric Spellcraft thesis (Wizards+). Share actions but get access to a special act together, modified based on your class (example below). Shares class abilities and resources. Health is shared, but only the character hit "goes down" when taken to 0 - the other remains at 1 but gains the Fleeing Condition and can only run away or run to try to stabilise the twin. The GM may allow taking feats from the Sniping Duo, Mystic Duo or even Summoner, modified if need be.
...
- **Wired Bomb.** 
  ...
  Requires Chemical Poliorcetica (Alchemists+).
  When you place or attach a bomb instead of Striking with it, you can arm it with a manual detonation method. Designate a trigger. The bomb no longer detonates from a creature moving through its space or using an action with a manipulate trait on it. If the trigger you designated occurs before the start of your next turn, you can detonate the bomb as a reaction. 
...
- **Van and Foggle**. 
	In adventures where Stamina (see [[Houserules & Homebrew]]) plays a role, these serve as easy ways to regain stamina points. 
